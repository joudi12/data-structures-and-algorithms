...
const createServer = () => {
  ...
  var server = app.listen(3301, function () {
    ...
    app.get('/hello', handelgreet);
    app.get('/aboutme', handelaboutme);
    app.get('/favoritefoods', handelfavorit);
    app.get('/*', handelerorr);

    function handelgreet(req,res){
      res.status(200).send('welcom here');
    }
    function handelaboutme(req,res){
      res.status(200).send('i am joudi awameh my jorny with coding start when i was 19 and i like it so mush ');
    }
    function handelfavorit(req,res){
      res.status(200).send(['pizza' , 'taboly' ,'Wilngy']);
    }
    function handelerorr(req,res){
...
return input.reduce((acc, val, idx) => {
  acc += input[idx].reduce((accum, value) => {
    accum += value;
    return accum;
  }, 0)
  return acc;
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let divisByFive = input.map((val, idx) => {
    return input[idx].filter(value => {
